    ...
    )
{
  ...
  while(map->Signature == MEMORY_MAP_SIGNATURE) {
    ...
    countEntryNo++;
    map = SB_CR(map->Link.ForwardLink, MEMORY_MAP, Link);
  }
  ...
  if(isValid==TRUE) {
    while(map->Signature == MEMORY_MAP_SIGNATURE) {
      ...
      countEntryNo++;
      map = SB_CR(map->Link.BackLink, MEMORY_MAP, Link);
    }
  }
    ...
    )
{
  ...
  map = SB_CR(gMemoryMap->ForwardLink, MEMORY_MAP, Link);
  for(; map->Signature == MEMORY_MAP_SIGNATURE; map = SB_CR(map->Link.ForwardLink, MEMORY_MAP, Link)) {
