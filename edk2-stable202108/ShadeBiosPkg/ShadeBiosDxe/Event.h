    ...
    )
{
  ...
  while(evt->Signature == EVENT_SIGNATURE) {
    evt = SB_CR(evt->Timer.Link.BackLink, IEVENT, Timer.Link);
  }
...
{
  IEVENT *evt = SB_CR(gEfiTimerList->ForwardLink, IEVENT, Timer.Link);
  ...
  while(evt->Signature == EVENT_SIGNATURE) {
    ...
    evt = SB_CR(evt->Timer.Link.ForwardLink, IEVENT, Timer.Link);
  }
}
